...
def evaluate_board(board):
    ...
    # ==================================================
    # ΔΙΠΛΩΜΕΝΟΙ ΠΥΡΓΟΙ (ΜΟΝΟ ΚΑΘΕΤΑ ΣΤΗΝ ΙΔΙΑ ΣΤΗΛΗ)
    # ==================================================
    # Λευκοί Πύργοι
    if w_r_count == 2:
        w_rk_squares = list(chess.SquareSet(w_rooks))
        # chess.square_file(sq) επιστρέφει τη στήλη (0-7). Αν οι στήλες είναι ίδιες, είναι κάθετα διπλωμένοι.
        if chess.square_file(w_rk_squares[0]) == chess.square_file(w_rk_squares[1]):
            score += 25  # Αυξήσαμε ελαφρώς το μπόνους σε +25 για να το επιδιώκει περισσότερο
...
def minimax(board, depth, alpha, beta, maximizing, nodes, start_time, time_limit, ply=0, best_move_hint=None, interrupt_queue=None):
    ...
    # ΠΕΙΡΑΜΑ: Ακραίο Null Move Pruning με R=3 από βάθος 4 και πάνω
    ...
    if maximizing:
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if depth <= 3 and not in_check and not is_capture and not is_promo:
                # ΠΕΙΡΑΜΑ: Ακραίο κατέβασμα στο 80 για μέγιστη ταχύτητα
                futility_margin = depth * 80
            ...
            if board.is_repetition(3):
                ...
                # --- AGGRESSIVE LMR & PV SEARCH (ΠΕΙΡΑΜΑΤΙΚΟ) ---
                if moves_searched >= 2 and depth >= 3 and not in_check and not is_capture and not is_promo:
                    ...
                    # ΠΕΙΡΑΜΑ: Έξτρα μείωση βάθους ανάλογα με τη σειρά της κίνησης
                    if moves_searched >= 8:
                        reduction += 2
                    ...
                    # PV Search: Έλεγχος πρώτα με στενό παράθυρο (Null Window)
                    ...
                    # Αν αποδειχθεί καλή κίνηση, ξαναψάχνουμε κανονικά
                ...
                # PV Search: Αν δεν είναι η πρώτη κίνηση, στενεύουμε το παράθυρο
                    ...
                    # Η πρώτη κίνηση (PV κίνηση) ψάχνεται πάντα κανονικά
                    score, line = minimax(board, depth - 1, alpha, beta, False, nodes, start_time, time_limit, ply=ply+1, interrupt_queue=interrupt_queue)
