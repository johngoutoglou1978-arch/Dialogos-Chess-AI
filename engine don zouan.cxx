...
int evaluate_board(const Board& board) {
    ...
    for (int sq = 0; sq < 64; ++sq) {
        ...
        // Ορισμός αξιών και αυτόματη επιλογή PST (Middlegame ή Endgame)
        if (p.type() == PieceType::PAWN) {
            p_val = 100; 
            pst = is_endgame ? pawn_endgame_pst : pawn_pst;
        } else if (p.type() == PieceType::KNIGHT) {
            p_val = 320; 
            pst = is_endgame ? knight_endgame_pst : knight_pst;
        } else if (p.type() == PieceType::BISHOP) {
            p_val = 330; 
            pst = is_endgame ? bishop_endgame_pst : bishop_pst;
        } else if (p.type() == PieceType::ROOK) {
            p_val = 500; 
            pst = is_endgame ? rook_endgame_pst : rook_pst;
        } else if (p.type() == PieceType::QUEEN) {
            p_val = 900; 
            pst = is_endgame ? queen_endgame_pst : queen_pst;
    ...
    for (int file = 0; file < 8; ++file) {
        if (white_pawns_in_file[file] > 1) {
            score -= (white_pawns_in_file[file] - 1) * 30;
        }
        if (black_pawns_in_file[file] > 1) {
            score += (black_pawns_in_file[file] - 1) * 30;
        }
    }
    
    // =========================================================================
    // 3. Η ΜΕΓΑΛΗ ΓΝΩΣΗ: ΣΤΡΑΓΓΑΛΙΣΜΟΣ & ΠΕΡΙΟΡΙΣΜΟΣ ΚΙΝΗΤΙΚΟΤΗΤΑΣ ΑΝΤΙΠΑΛΟΥ
    // =========================================================================
    ...
    // --- WHITE MOBILITY (Λευκά Κομμάτια) ---
    chess::Bitboard w_knights = board.pieces(chess::PieceType::KNIGHT, chess::Color::WHITE);
    while (w_knights) {
        chess::Square sq = w_knights.pop();
        white_mobility += (chess::attacks::knight(sq) & white_filter).count();
    }
    chess::Bitboard w_bishops = board.pieces(chess::PieceType::BISHOP, chess::Color::WHITE);
    while (w_bishops) {
        chess::Square sq = w_bishops.pop();
        white_mobility += (chess::attacks::bishop(sq, occ) & white_filter).count();
    }
    chess::Bitboard w_rooks = board.pieces(chess::PieceType::ROOK, chess::Color::WHITE);
    while (w_rooks) {
        chess::Square sq = w_rooks.pop();
        white_mobility += (chess::attacks::rook(sq, occ) & white_filter).count();
    }
    chess::Bitboard w_queens = board.pieces(chess::PieceType::QUEEN, chess::Color::WHITE);
    while (w_queens) {
        chess::Square sq = w_queens.pop();
        white_mobility += (chess::attacks::queen(sq, occ) & white_filter).count();
    }

    // --- BLACK MOBILITY (Μαύρα Κομμάτια) ---
    chess::Bitboard b_knights = board.pieces(chess::PieceType::KNIGHT, chess::Color::BLACK);
    while (b_knights) {
        chess::Square sq = b_knights.pop();
        black_mobility += (chess::attacks::knight(sq) & black_filter).count();
    }
    chess::Bitboard b_bishops = board.pieces(chess::PieceType::BISHOP, chess::Color::BLACK);
    while (b_bishops) {
        chess::Square sq = b_bishops.pop();
        black_mobility += (chess::attacks::bishop(sq, occ) & black_filter).count();
    }
    chess::Bitboard b_rooks = board.pieces(chess::PieceType::ROOK, chess::Color::BLACK);
    while (b_rooks) {
        chess::Square sq = b_rooks.pop();
        black_mobility += (chess::attacks::rook(sq, occ) & black_filter).count();
    }
    chess::Bitboard b_queens = board.pieces(chess::PieceType::QUEEN, chess::Color::BLACK);
    while (b_queens) {
        chess::Square sq = b_queens.pop();
        black_mobility += (chess::attacks::queen(sq, occ) & black_filter).count();
    }
...
int alphabeta(int alpha, int beta, int depth, chess::Board& board, int ply, chess::Move& best_move) {
    ...
    // --- [STOCKFISH TECH] REVERSE FUTILITY PRUNING (RFP) ---
    ...
    if (depth >= 3 && !in_check && ply > 0) {
        board.makeNullMove();
        int reduction = 3 + (depth / 4); // Stockfish-style δυναμικό reduction
        chess::Move dummy_move;
        int null_score = -alphabeta(-beta, -beta + 1, depth - 1 - reduction, board, ply + 1, dummy_move);
    ...
    // --- ΤΟ ΔΙΚΟ ΣΟΥ MOVE ORDERING LOOP (Διορθωμένο) ---
    int move_scores[256] = {0}; 
    for (size_t i = 0; i < moves.size(); i++) {
        ...
        if (board.isCapture(moves[i])) {
            ...
            // Διόρθωση των Killer Moves με βάση το δικό σας unordered_set
            if (ply < 64 && KILLERS[ply].count(uci)) {
                move_scores[i] = 9000;
            }
            // Ενσωμάτωση του History Heuristic στη βαθμολόγηση των quiet κινήσεων
            if (HISTORY.count(uci)) {
                move_scores[i] += HISTORY[uci];
            }
        }
    }
    ...
    for (size_t i = 0; i < moves.size(); i++) {
        ...
        // Έλεγχος promotion με βάση τους κανόνες του σκακιού (Zero-API Dependency)
        auto moving_piece = board.at<chess::PieceType>(mv.from());
        int target_rank = mv.to().rank(); 
        bool is_promotion = (moving_piece == chess::PieceType::PAWN) && (target_rank == 0 || target_rank == 7);
        ...
        if (search_pv) {
            ...
            if (depth >= 3 && legal_moves_searched > 3 && !board.inCheck() && !is_capture && !is_promotion) {
                lmr_reduction = 1 + (legal_moves_searched / 8); // Stockfish-style επιθετικό LMR
                
                if (HISTORY.count(uci) && HISTORY[uci] < 0) {
                    lmr_reduction++; // Έξτρα μείωση αν η κίνηση έχει κακό ιστορικό
                }
                if (lmr_reduction >= depth) lmr_reduction = depth - 1;
            }
        ...
        if (score >= beta) {
            if (!is_capture && ply < 64) {
                // Διορθωμένο insertion για το δικό σας unordered_set
                KILLERS[ply].insert(uci);
                
                // --- [STOCKFISH TECH] HISTORY BONUS & PENALTY ---
                HISTORY[uci] += depth * depth; // Bonus στην κίνηση που έκανε το cutoff
                
                // Penalty σε όλες τις προηγούμενες quiet κινήσεις που αποτύχανε
                for (size_t j = 0; j < i; j++) {
                    chess::Move bad_mv = moves[j];
                    if (!board.isCapture(bad_mv)) {
                        HISTORY[chess::uci::moveToUci(bad_mv)] -= depth * depth;
                    }
                }
            }
    ...
    while (true) {
        ...
        if (board.sideToMove() == ai_side) {
            ...
            while (true) {
                ...
                // Μορφοποίηση με 2 δεκαδικά ψηφία στο Live Output
            ...
            if (best_move_full != Move::NO_MOVE) {
                ...
                // Μορφοποίηση με 2 δεκαδικά ψηφία στην τελική εκτύπωση της κίνησης
            ...
            if (move_str == "undo") {
                if (game_history.size() >= 2) {
                    ...
                    cout << "\033[33m⏪ Έγिने αναίρεση των 2 τελευταίων κινήσεων!\033[0m\n";
                } else {
