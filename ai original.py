...
def evaluate_board(board):
    ...
    DOUBLED_PAWN_PENALTY = 20  # Ποινή σε centipawns
    ...
    PROPHYLAXIS_BONUS = 15  # Μπόνους σε centipawns

    # --- ΓΙΑ ΤΑ ΛΕΥΚΑ (Αποτροπή στο g4 και b4) ---
    # Αν το λευκό πιόνι είναι στο h3 (τετράγωνο 23 / chess.H3)
    if board.piece_at(chess.H3) == chess.Piece(chess.PAWN, chess.WHITE):
        # Ελέγχουμε αν μαύρος ίππος ή αξιωματικός απειλεί/ελέγχει το g4
        if board.is_attacked_by(chess.BLACK, chess.G4):
            score += PROPHYLAXIS_BONUS
            
    # Αν το λευκό πιόνι είναι στο a3 (τετράγωνο 16 / chess.A3)
    if board.piece_at(chess.A3) == chess.Piece(chess.PAWN, chess.WHITE):
        # Ελέγχουμε αν μαύρος ίππος ή αξιωματικός απειλεί/ελέγχει το b4
        if board.is_attacked_by(chess.BLACK, chess.B4):
            score += PROPHYLAXIS_BONUS

    # --- ΓΙΑ ΤΑ ΜΑΥΡΑ (Αποτροπή στο g5 και b5) ---
    # Αν το μαύρο πιόνι είναι στο h6 (τετράγωνο 47 / chess.H6)
    if board.piece_at(chess.H6) == chess.Piece(chess.PAWN, chess.BLACK):
        # Ελέγχουμε αν λευκός ίππος ή αξιωματικός απειλεί/ελέγχει το g5
        if board.is_attacked_by(chess.WHITE, chess.G5):
            score -= PROPHYLAXIS_BONUS
            
    # Αν το μαύρο πιόνι είναι στο a6 (τετράγωνο 40 / chess.A6)
    if board.piece_at(chess.A6) == chess.Piece(chess.PAWN, chess.BLACK):
        # Ελέγχουμε αν λευκός ίππος ή αξιωματικός απειλεί/ελέγχει το b5
        if board.is_attacked_by(chess.WHITE, chess.B5):
            score -= PROPHYLAXIS_BONUS
    ...
    # --- ΓΙΑ ΤΑ ΛΕΥΚΑ ---
    # 1. Πιόνι στο g4 κλοτσάει Αξιωματικό στο h5, ΜΕ υποστήριξη από πιόνι στο h3
    if board.piece_at(chess.G4) == chess.Piece(chess.PAWN, chess.WHITE):
        if board.piece_at(chess.H3) == chess.Piece(chess.PAWN, chess.WHITE):
            if board.piece_at(chess.H5) == chess.Piece(chess.BISHOP, chess.BLACK):
                score += CHASING_BONUS

    # 2. Πιόνι στο b4 κλοτσάει Αξιωματικό στο a5, ΜΕ υποστήριξη από πιόνι στο a3
    if board.piece_at(chess.B4) == chess.Piece(chess.PAWN, chess.WHITE):
        if board.piece_at(chess.A3) == chess.Piece(chess.PAWN, chess.WHITE):
            if board.piece_at(chess.A5) == chess.Piece(chess.BISHOP, chess.BLACK):
                score += CHASING_BONUS

    # --- ΓΙΑ ΤΑ ΜΑΥΡΑ ---
    # 1. Πιόνι στο g5 κλοτσάει Αξιωματικό στο h4, ΜΕ υποστήριξη από πιόνι στο h6
    if board.piece_at(chess.G5) == chess.Piece(chess.PAWN, chess.BLACK):
        if board.piece_at(chess.H6) == chess.Piece(chess.PAWN, chess.BLACK):
            if board.piece_at(chess.H4) == chess.Piece(chess.BISHOP, chess.WHITE):
                score -= CHASING_BONUS

    # 2. Πιόνι στο b5 κλοτσάει Αξιωματικό στο a4, ΜΕ υποστήριξη από πιόνι στο a6
    if board.piece_at(chess.B5) == chess.Piece(chess.PAWN, chess.BLACK):
        if board.piece_at(chess.A6) == chess.Piece(chess.PAWN, chess.BLACK):
            if board.piece_at(chess.A4) == chess.Piece(chess.BISHOP, chess.WHITE):
                score -= CHASING_BONUS
...
def interactive_gameplay():
    ...
    while not board.is_game_over():
        ...
        if board.turn == ai_side:
            ...
            # === ΝΕΟΣ ΜΗΧΑΝΙΣΜΟΣ ΕΛΕΓΧΟΥ ΜΝΗΜΗΣ TT ===
            tt_count = len(transposition_table)
            fill_percentage = (tt_count / MAX_TT_SIZE) * 100

            # Επιλογή χρώματος κειμένου ανάλογα με το ποσοστό
            if fill_percentage < 70:
                mem_color = "\033[32m🟢" # Πράσινο
            elif fill_percentage < 85:
                mem_color = "\033[33m⚠️ " # Κίτρινο
            else:
                mem_color = "\033[31m🚨 ΚΙΝΔΥΝΟΣ! " # Κόκκινο Προειδοποιητικό
            ...
            # Εντολή για ακαριαίο άδειασμα του TT χωρίς διακοπή παιχνιδιού
            if move_str == "clear":
    ...
    # 1. Εμφανίζουμε τη σκακιέρα με την τελευταία κίνηση (το ματ)
    print_board_fidelity_relief(board)

    # 2. Τυπώνουμε το τελικό αποτέλεσμα
