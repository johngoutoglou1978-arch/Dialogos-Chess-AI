...
def evaluate_board(board):
    ...
    # ==================================================
    # ROOK STRATEGY (Updated with Endgame Knowledge)
    # ==================================================
    
    # 1. Υπολογισμός Περασμένων Πιόνιων (Passed Pawns) με Bitboards μόνο στο Φινάλε
    w_passed_pawns = 0
    b_passed_pawns = 0
    
    if endgame:  # Εδώ χρησιμοποιείτε τη δική σας μεταβλητή για το φινάλε
        # Εύρεση λευκών περασμένων πιονιών
        for sq in chess.SquareSet(w_pawns):
            f = chess.square_file(sq)
            r = chess.square_rank(sq)
            mask = chess.BB_FILES[f] | (chess.BB_FILES[f-1] if f > 0 else 0) | (chess.BB_FILES[f+1] if f < 7 else 0)
            if not (b_pawns & mask & (0xFFFFFFFFFFFFFFFF << (8 * (r + 1)))):
                w_passed_pawns |= chess.BB_SQUARES[sq]
                
        # Εύρεση μαύρων περασμένων πιονιών
        for sq in chess.SquareSet(b_pawns):
            f = chess.square_file(sq)
            r = chess.square_rank(sq)
            mask = chess.BB_FILES[f] | (chess.BB_FILES[f-1] if f > 0 else 0) | (chess.BB_FILES[f+1] if f < 7 else 0)
            if not (w_pawns & mask & (0xFFFFFFFFFFFFFFFF >> (8 * (8 - r)))):
                b_passed_pawns |= chess.BB_SQUARES[sq]
    ...
    for r_sq in chess.SquareSet(w_rooks):
        ...
        # 7η Γραμμή: Στο φινάλε δίνει μεγαλύτερο μπόνους (40 αντί για 25)
        if r_idx == 6: 
            score += 40 if endgame else 25
            
        # Ανοιχτές και Ημιανοιχτές Στήλες (Κρατάμε τη δική σας λογική με bitmasks)
        if not (all_pawns & f_mask): 
            score += 20
        elif not (w_pawns & f_mask): 
            score += 12
            
        # Διαχωρισμός Μέσου Παιχνιδιού / Φινάλε
        if not endgame:
            # Στο Μέσο: Μπόνους για τον έλεγχο των κεντρικών στηλών d και e
            if f_idx in (3, 4): score += 10
        else:
            # Στο Φινάλε: Κανόνας Tarrasch (Πύργος πίσω από περασμένα πιόνια στην ίδια στήλη)
            all_passed_in_file = (w_passed_pawns | b_passed_pawns) & f_mask
            for p_sq in chess.SquareSet(all_passed_in_file):
                if chess.square_rank(p_sq) > r_idx:  # Το πιόνι είναι πιο μπροστά από τον πύργο
                    score += 35  # Μπόνους δραστηριότητας πύργου στο φινάλε
                    break
    ...
    for r_sq in chess.SquareSet(b_rooks):
        ...
        # 2η Γραμμή: Στο φινάλε δίνει μεγαλύτερο μπόνους
        if r_idx == 1: 
            score -= 40 if endgame else 25
            
        # Ανοιχτές και Ημιανοιχτές Στήλες
        if not (all_pawns & f_mask): 
            score -= 20
        elif not (b_pawns & f_mask): 
            score -= 12
        ...
        if not endgame:
            ...
            # Στο Φινάλε: Κανόνας Tarrasch
            all_passed_in_file = (w_passed_pawns | b_passed_pawns) & f_mask
            for p_sq in chess.SquareSet(all_passed_in_file):
                if chess.square_rank(p_sq) < r_idx:  # Το πιόνι είναι πιο μπροστά (προς τα κάτω) από τον πύργο
                    score -= 35
                    break
...
def minimax(board, depth, alpha, beta, maximizing, nodes, start_time, time_limit, ply=0, best_move_hint=None, interrupt_queue=None):
    ...
    # Προσαρμοστικό Null Move R (Μεγαλύτερο κλάδεμα στα μεγάλα βάθη για +20)
    R = 3 if depth > 8 else 2 
    ...
    if depth <= 3 and not in_check:
        margin = depth * 120
        if maximizing and (static_eval - margin >= beta):
            return beta, []
        elif not maximizing and (static_eval + margin <= alpha):
            return alpha, []
    ...
    if maximizing:
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if board.is_repetition(3):
                ...
                # =============================================================
                # PRINCIPAL VARIATION SEARCH (PVS) & LMR (Λευκά)
                # =============================================================
                if moves_searched >= 2 and depth >= 3 and not in_check and not is_capture and not is_promo:
                    # Πιο επιθετικό LMR (διαιρέτης 1.2 αντί για 1.5)
                    base_reduction = math.log(depth) * math.log(moves_searched + 1) / 1.2
                    reduction = int(0.5 + base_reduction)
                    if depth > 8:
                        reduction += 2 
                    reduction = max(1, min(reduction, depth - 1))
                    
                    # Αναζήτηση με Μειωμένο Βάθος και Μηδενικό Παράθυρο (Zero Window)
                    ...
                    # Αν αποτύχει, αναζήτηση με πλήρες βάθος αλλά πάλι Zero Window
                    ...
                    # Zero Window για όλες τις κινήσεις μετά την 1η
                    ...
                    # Η 1η κίνηση (PV) ψάχνεται πάντα κανονικά
                    score, line = minimax(board, depth - 1, alpha, beta, False, nodes, start_time, time_limit, ply=ply+1, interrupt_queue=interrupt_queue)
    ...
    # --- MINIMIZING PLAYER (Μαύρα) ---
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if board.is_repetition(3):
                ...
                # =============================================================
                # PRINCIPAL VARIATION SEARCH (PVS) & LMR (Μαύρα)
                # =============================================================
                if moves_searched >= 2 and depth >= 3 and not in_check and not is_capture and not is_promo:
                    base_reduction = math.log(depth) * math.log(moves_searched + 1) / 1.2
                    reduction = int(0.5 + base_reduction)
                    ...
                    # Zero Window αναζήτηση για τα μαύρα (παράθυρο: beta-1 έως beta)
                    ...
                    # Η 1η κίνηση (PV) ψάχνεται κανονικά
                    score, line = minimax(board, depth - 1, alpha, beta, True, nodes, start_time, time_limit, ply=ply+1, interrupt_queue=interrupt_queue)
