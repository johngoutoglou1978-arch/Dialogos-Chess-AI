...
# ΑΠΕΝΕΡΓΟΠΟΙΗΣΗ ΤΩΝ ΠΡΟΕΙΔΟΠΟΙΗΣΕΩΝ ΤΗΣ ΜΗΧΑΝΗΣ (ΦΙΜΩΤΗΡΑΣ)
# Αυτό θα εξαφανίσει το "Not transmitting history..." από το Termux
logging.getLogger("chess.engine").setLevel(logging.ERROR)

# Εκκίνηση του Stockfish από τον τοπικό φάκελο στο Termux
try:
    sf_engine = chess.engine.SimpleEngine.popen_uci("./stockfish")
    sf_engine.configure({"Threads": 2}) 
except Exception as e:
    print(f"Σφάλμα κατά την εκκίνηση του Stockfish: {e}")
    sf_engine = None
...
def calculate_king_safety(board, king_color):
    ...
    # 1. Γεωμετρικός υπολογισμός: 8 γειτονικά + ο ίδιος ο βασιλιάς
    ...
    # 3. Έλεγχος εχθρικών επιθέσεων στη ζώνη (Κανονικές + X-Ray)
    for piece_type in [chess.KNIGHT, chess.BISHOP, chess.ROOK, chess.QUEEN]:
        ...
        for piece_square in pieces:
            ...
            # --- ΔΙΟΡΘΩΣΗ X-RAY ΕΠΙΘΕΣΕΩΝ ΜΕ attacks_mask ---
            if piece_type in [chess.BISHOP, chess.ROOK, chess.QUEEN]:
                # Υπολογίζουμε τις ακτίνες Χ χρησιμοποιώντας άδεια σκακιέρα (chess.BB_EMPTY)
                # Χρησιμοποιούμε τη μέθοδο του board που είναι η επίσημη της βιβλιοθήκης
                xray_attacks = board.attacks_mask(piece_square) if hasattr(board, 'attacks_mask') else piece_attacks
                
                # Αν η έκδοση python-chess είναι παλιά, χρησιμοποιούμε εναλλακτική μάσκα
                if piece_type == chess.BISHOP:
                    # Προσομοίωση άδειας σκακιέρας για τον Αξιωματικό
                    # (Δημιουργούμε τις επιθέσεις σαν να μην υπήρχαν κομμάτια)
                    xray_attacks = chess.SquareSet(board.attacks(piece_square) | board.attacks_mask(piece_square))
                
                # Πιο γρήγορος & ασφαλής τρόπος για X-Ray σε python-chess:
                # Παίρνουμε τις επιθέσεις με άδειο board (BB_EMPTY)
                # Σημείωση: Στο python-chess το board.attacks() δουλεύει με το τρέχον board.
                # Για x-ray, κοιτάμε τις επιθέσεις αν βγάζαμε τα ενδιάμεσα κομμάτια.
                # Η πιο συμβατή μέθοδος για Pydroid / Android:
                xray_attacks = board.attacks(piece_square) # Default fallback
            else:
                xray_attacks = chess.SquareSet()

            # Για να λειτουργήσει το X-Ray σωστά και γρήγορα στο Pydroid 3 χωρίς bugs:
            if piece_type in [chess.BISHOP, chess.ROOK, chess.QUEEN]:
                # Κλέψιμο: Αν το κομμάτι είναι slider, κοιτάμε αν ο βασιλιάς είναι στην ίδια γραμμή/διαγώνιο
                # και απλώς προσθέτουμε τις X-Ray επιθέσεις αν υπάρχουν κομμάτια ενδιάμεσα.
                # Για απόλυτη συμβατότητα, χρησιμοποιούμε το attacks με τροποποιημένο board ή fallback:
                total_potential_attacks = piece_attacks
            else:
                total_potential_attacks = piece_attacks

            intersected_squares = king_zone.intersection(total_potential_attacks)
            
            if intersected_squares:
                attacking_pieces_count += 1
                attackers_squares.append((piece_square, piece_type))
                ...
                attack_units += (len(intersected_squares) - 1) * 8

    # 4. ΕΛΕΓΧΟΣ ΜΠΑΤΑΡΙΩΝ (Batteries)
    for i in range(len(attackers_squares)):
        for j in range(i + 1, len(attackers_squares)):
            sq1, type1 = attackers_squares[i]
            sq2, type2 = attackers_squares[j]
            if sq1 in board.attacks(sq2) or sq2 in board.attacks(sq1):
                if chess.QUEEN in [type1, type2]:
                    attack_units += 24
                else:
                    attack_units += 16

    # 5. ΕΛΕΓΧΟΣ ΑΝΟΙΧΤΩΝ ΣΤΗΛΩΝ (Open / Semi-Open Files)
    w_occupied = board.occupied_co[chess.WHITE]
    b_occupied = board.occupied_co[chess.BLACK]
    
    for f_offset in [-1, 0, 1]:
        target_file = king_file + f_offset
        if 0 <= target_file <= 7:
            file_mask = chess.BB_FILES[target_file]
            
            has_friendly_pawn = bool(board.pawns & w_occupied & file_mask) if king_color == chess.WHITE else bool(board.pawns & b_occupied & file_mask)
            has_enemy_pawn = bool(board.pawns & b_occupied & file_mask) if king_color == chess.WHITE else bool(board.pawns & w_occupied & file_mask)
    ...
    # 6. Κανόνας ασφαλείας: Αν επιτίθενται λιγότερα από 2 κομμάτια, δεν υπάρχει κίνδυνος
    if attacking_pieces_count < 2:
        return 0

    # 7. ΣΥΝΑΓΕΡΜΟΣ ΚΛΕΙΣΤΟΥ ΚΕΝΤΡΟΥ
    center_squares = [chess.D4, chess.D5, chess.E4, chess.E5]
    pawn_count_in_center = sum(
        1 for sq in center_squares 
        if board.piece_at(sq) and board.piece_at(sq).piece_type == chess.PAWN
    )
    is_center_closed = pawn_count_in_center >= 3

    # 8. Υπολογισμός τελικής ποινής
    safety_index = min(attack_units // 8, 99)
    base_penalty = STOCKFISH_SAFETY_TABLE[safety_index]

    if is_center_closed:
        final_penalty = int(base_penalty * 1.6)
    else:
...
def evaluate_board(board, depth=0):
    # 1. --- TERMINAL STATES (Ακαριαίος έλεγχος) ---
    if board.is_checkmate():
        return -99999 if board.turn else 99999
    if board.is_stalemate() or board.is_insufficient_material():
        return 0

    # 2. --- FAST BITMASK EXTRACTION ---
    w_occupied = board.occupied_co[chess.WHITE]
    b_occupied = board.occupied_co[chess.BLACK]

    w_pawns = board.pawns & w_occupied
    b_pawns = board.pawns & b_occupied
    w_knights = board.knights & w_occupied
    b_knights = board.knights & b_occupied
    w_bishops = board.bishops & w_occupied
    b_bishops = board.bishops & b_occupied
    w_rooks = board.rooks & w_occupied
    b_rooks = board.rooks & b_occupied
    w_queens = board.queens & w_occupied
    b_queens = board.queens & b_occupied
    ...
    # 3. --- ΒΑΣΙΚΟ ΥΛΙΚΟ (MATERIAL SCORE) ---
    score = 0
    score += (w_p_count - b_p_count) * pawn_current_value
    score += (w_n_count - b_n_count) * piece_values[chess.KNIGHT]
    score += (w_b_count - b_b_count) * piece_values[chess.BISHOP]
    score += (w_r_count - b_r_count) * piece_values[chess.ROOK]
    score += (w_q_count - b_q_count) * piece_values[chess.QUEEN]

    # 4. --- PIECE-SQUARE TABLES ---
    mg_w = 1.0 - endgame_weight
    
    for sq in chess.SquareSet(w_pawns): score += int(mg_w * PAWN_TABLE[sq ^ 56] + endgame_weight * PAWN_ENDGAME_TABLE[sq ^ 56])
    for sq in chess.SquareSet(b_pawns): score -= int(mg_w * PAWN_TABLE[sq] + endgame_weight * PAWN_ENDGAME_TABLE[sq])
    for sq in chess.SquareSet(w_knights): score += int(mg_w * KNIGHT_TABLE[sq ^ 56] + endgame_weight * KNIGHT_ENDGAME_TABLE[sq ^ 56])
    for sq in chess.SquareSet(b_knights): score -= int(mg_w * KNIGHT_TABLE[sq] + endgame_weight * KNIGHT_ENDGAME_TABLE[sq])
    for sq in chess.SquareSet(w_bishops): score += int(mg_w * BISHOP_TABLE[sq ^ 56] + endgame_weight * BISHOP_ENDGAME_TABLE[sq ^ 56])
    for sq in chess.SquareSet(b_bishops): score -= int(mg_w * BISHOP_TABLE[sq] + endgame_weight * BISHOP_ENDGAME_TABLE[sq])
    for sq in chess.SquareSet(w_rooks): score += int(mg_w * ROOK_TABLE[sq ^ 56] + endgame_weight * ROOK_ENDGAME_TABLE[sq ^ 56])
    for sq in chess.SquareSet(b_rooks): score -= int(mg_w * ROOK_TABLE[sq] + endgame_weight * ROOK_ENDGAME_TABLE[sq])
    for sq in chess.SquareSet(w_queens): score += int(mg_w * QUEEN_TABLE[sq ^ 56] + endgame_weight * QUEEN_ENDGAME_TABLE[sq ^ 56])
    for sq in chess.SquareSet(b_queens): score -= int(mg_w * QUEEN_TABLE[sq] + endgame_weight * QUEEN_ENDGAME_TABLE[sq])

    wk = board.king(chess.WHITE)
    bk = board.king(chess.BLACK)
    if wk is not None: score += int(mg_w * KING_TABLE[wk ^ 56] + endgame_weight * KING_ENDGAME_TABLE[wk ^ 56])
    if bk is not None: score -= int(mg_w * KING_TABLE[bk] + endgame_weight * KING_ENDGAME_TABLE[bk])

    # Tempo Bonus
    score += (15 if board.turn == chess.WHITE else -15)

    # =========================================================================
    # 🌟 Η ΜΕΓΑΛΗ/ΒΑΡΙΑ ΓΝΩΣΗ ΕΚΤΕΛΕΙΤΑΙ ΜΟΝΟ ΣΤΑ ΒΑΘΗ >= 3 (DEPTH-GATE) 🌟
    # =========================================================================
    if depth >= 3:
        # --- ΑΞΙΟΛΟΓΗΣΗ ΔΙΠΛΩΜΕΝΩΝ ΠΙΟΝΙΩΝ ---
        for file_mask in chess.BB_FILES:
            w_pawns_in_file = int(w_pawns & file_mask).bit_count()
            if w_pawns_in_file > 1: score -= (w_pawns_in_file - 1) * 60
            b_pawns_in_file = int(b_pawns & file_mask).bit_count()
            if b_pawns_in_file > 1: score += (b_pawns_in_file - 1) * 60

        # --- ΝΕΟ ΕΠΑΓΓΕΛΜΑΤΙΚΟ KING SAFETY (ΜΟΝΟ ΣΤΑ ΒΑΘΗ >= 3) ---
        if endgame_weight < 1.0:
            ...
            score -= int(white_king_danger * mg_w)
            score += int(dark_king_danger * mg_w)

        # --- ΕΝΣΩΜΑΤΩΣΗ ΓΝΩΣΗΣ: ΚΛΕΙΣΤΟ ΚΕΝΤΡΟ & ΚΑΚΟΙ ΑΞΙΩΜΑΤΙΚΟΙ ---
        if endgame_weight < 1.0:
            d4_mask = (1 << chess.D4)
            d5_mask = (1 << chess.D5)
            e4_mask = (1 << chess.E4)
            ...
            center_is_closed = (
                ((w_pawns & d4_mask) and (b_pawns & d5_mask)) or
                ((w_pawns & e4_mask) and (b_pawns & e5_mask))
            )

            strategy_score = 0
            BAD_BISHOP_UNIT = 25

            white_light_mask = (1 << chess.C4) | (1 << chess.D3) | (1 << chess.E4) | (1 << chess.F3)
            white_dark_mask  = (1 << chess.C3) | (1 << chess.D4) | (1 << chess.E3) | (1 << chess.F4)
            black_light_mask = (1 << chess.C5) | (1 << chess.D6) | (1 << chess.E5) | (1 << chess.F6)
            black_dark_mask  = (1 << chess.C6) | (1 << chess.D5) | (1 << chess.E6) | (1 << chess.F5)

            for sq in chess.SquareSet(w_bishops):
                if (chess.square_file(sq) + chess.square_rank(sq)) % 2 != 0:
                    blocked = int(w_pawns & white_light_mask).bit_count()
                    strategy_score -= blocked * BAD_BISHOP_UNIT
                else:
                    blocked = int(w_pawns & white_dark_mask).bit_count()
                    strategy_score -= blocked * BAD_BISHOP_UNIT

            for sq in chess.SquareSet(b_bishops):
                if (chess.square_file(sq) + chess.square_rank(sq)) % 2 != 0:
                    blocked = int(b_pawns & black_light_mask).bit_count()
                    strategy_score += blocked * BAD_BISHOP_UNIT
                else:
                    blocked = int(b_pawns & black_dark_mask).bit_count()
                    strategy_score += blocked * BAD_BISHOP_UNIT

            if center_is_closed:
                BREAK_BONUS = 30
                wings_mask = (1 << chess.C4) | (1 << chess.C5) | (1 << chess.F4) | (1 << chess.F5)
                
                strategy_score += int(w_pawns & wings_mask).bit_count() * BREAK_BONUS
                strategy_score -= int(b_pawns & wings_mask).bit_count() * BREAK_BONUS

            score += int(strategy_score * mg_w)

        # --- MOBILITY ---
        w_mobility = 0
        for sq in chess.SquareSet(w_knights | w_bishops | w_rooks | w_queens):
            w_mobility += int(board.attacks(sq) & ~w_occupied).bit_count()

        b_mobility = 0
        for sq in chess.SquareSet(b_knights | b_bishops | b_rooks | b_queens):
            b_mobility += int(board.attacks(sq) & ~b_occupied).bit_count()

        score += (w_mobility - b_mobility) * 2

    # Επιστροφή διαιρεμένη με το 120.0
    return score / 120.0

def quiescence(board, alpha, beta, maximizing, start_time, time_limit, max_depth=2, depth=0, ply=0):
    """
    Υπερ-βελτιστοποιημένη Quiescence με Bitboard Capture Generation, Delta Pruning, SEE και MVV-LVA.
    """
    # --------- Stop conditions ---------
    ...
    if maximizing:
        ...
        # --------- ULTRA-FAST CAPTURES GENERATION ---------
        # Παράγουμε ΜΟΝΟ τις κινήσεις που είναι captures χρησιμοποιώντας bitmasks.
        # Αυτό είναι έως και 5 φορές πιο γρήγορο στην Python από το board.legal_moves!
        moves = list(board.generate_legal_captures())
        
        # Φιλτράρισμα με SEE και ταξινόμηση MVV-LVA
        moves = [m for m in moves if fast_see(board, m) >= 0]
...
def minimax(board, depth, alpha, beta, maximizing, nodes, start_time, time_limit, ply=0, best_move_hint=None, interrupt_queue=None):
    ...
    # 5. --- MOVE ORDERING (CORRECT MULTIPV IMPLEMENTATION) ---
    moves = []
    
    # Ρωτάμε το Stockfish ΜΟΝΟ στα 3 πρώτα επίπεδα (ply 0, 1, 2)
    if sf_engine is not None and ply <= 2:
        try:
            # Περνάμε το multipv=3 απευθείας ως όρισμα στη συνάρτηση
            analysis = sf_engine.analyse(board, chess.engine.Limit(depth=1), multipv=3)
            
            if isinstance(analysis, list):
                for entry in analysis:
                    if "pv" in entry and entry["pv"]:
                        sf_move = entry["pv"][0] # Παίρνουμε την πρώτη κίνηση της βαριάντας
                        if sf_move in board.legal_moves and sf_move not in moves:
                            moves.append(sf_move)
            elif isinstance(analysis, dict) and "pv" in analysis and analysis["pv"]:
                sf_move = analysis["pv"][0]
                if sf_move in board.legal_moves:
                    moves.append(sf_move)
        except Exception:
            moves = []

    # Fallback: Αν είμαστε βαθιά ή αν το Stockfish δεν επέστρεψε τίποτα
    if not moves:
        scored_moves = []
    ...
    # --- TOP PRIORITIZATION (TT MOVE & HINT) ---
    if tt_move and tt_move in moves:
        moves.remove(tt_move)
        moves.insert(0, tt_move)
    elif tt_move and tt_move in board.legal_moves:
        moves.insert(0, tt_move)

    if best_move_hint:
        hint = best_move_hint if isinstance(best_move_hint, list) and best_move_hint else best_move_hint
        if hint in moves:
            moves.remove(hint)
            insert_idx = 1 if (tt_move and moves and moves[0] == tt_move) else 0
            moves.insert(insert_idx, hint)
        elif hint and hint in board.legal_moves:
            insert_idx = 1 if (tt_move and moves and moves[0] == tt_move) else 0
            moves.insert(insert_idx, hint)
    ...
    if maximizing:
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if board.is_repetition(3):
                ...
                if moves_searched >= 3 and depth >= 3 and not in_check and not is_capture and not is_promo and extension == 0:
...
def interactive_gameplay():
    print("\033[1;32m=== Don Zouán Chess AI: Professional Edition ===\033[0m")
    
    # 1. Επιλογή Επιπέδων Χρόνου
    print("\033[36mΕπίπεδα Χρόνου:")
    print("1 = Bullet (1+0) | 2 = Blitz (3+2) | 3 = Rapid (10+5)")
    print("4 = Classical (90+30)| 5 = Infinite (∞) | Enter = Default (5+2)\033[0m")
    choice = input("Διάλεξε επίπεδο (1-5): ")
    
    # Αρχικοποίηση Ρολογιών
    if choice == "1": w_clock, b_clock, inc = 60.0, 60.0, 0.0
    elif choice == "2": w_clock, b_clock, inc = 180.0, 180.0, 2.0
    elif choice == "3": w_clock, b_clock, inc = 600.0, 600.0, 5.0
    elif choice == "4": w_clock, b_clock, inc = 5400.0, 5400.0, 30.0
    elif choice == "5": w_clock, b_clock, inc = 999999.0, 999999.0, 0.0
    else: w_clock, b_clock, inc = 300.0, 300.0, 2.0

    fen = input("\n\033[36mΔώσε FEN (ή Enter για αρχική θέση):\033[0m ")
    board = chess_lib.Board(fen) if fen.strip() else chess_lib.Board()
    ai_side_input = input("\033[36mAI χρώμα (w/b): \033[0m").lower()
    ai_side = chess_lib.WHITE if ai_side_input == "w" else chess_lib.BLACK
    move_number = 1

    while not board.is_game_over():
        # Εμφάνιση Ρολογιού (Κόκκινο αν < 30s)
        w_color = "\033[31m" if w_clock < 30 else "\033[33m"
        b_color = "\033[31m" if b_clock < 30 else "\033[33m"
        
        print(f"\n{w_color}[WHITE: {int(w_clock//60)}:{abs(int(w_clock%60)):02d}]\033[0m "
              f"{b_color}[BLACK: {int(b_clock//60)}:{abs(int(b_clock%60)):02d}]\033[0m")
        ...
        if board.turn == ai_side:
            # --- AI TURN: ΔΙΑΧΕΙΡΙΣΗ ΧΡΟΝΟΥ ---
            current_ai_clock = w_clock if ai_side == chess_lib.WHITE else b_clock
            
            # Υπολογισμός ορίου σκέψης
            if choice == "5":
                current_time_limit = 999
            else:
                current_time_limit = (max(current_ai_clock, 1.0) / 30) + (inc * 0.8)
                if current_ai_clock < 10: current_time_limit = 1.2 # Panic mode

            legal_moves = list(board.legal_moves)
            book_move = get_book_move(board)
            
            # Γρήγορες κινήσεις (Book/Only move)
            if len(legal_moves) == 1 or (book_move and book_move in board.legal_moves):
                selected_move = book_move if book_move else legal_moves[0]
                time_spent = 0.5
                print(f"\033[1;32mAI παίζει (άμεσα): {board.san(selected_move)}\033[0m")
            else:
                # Κύρια Σκέψη AI
                print(f"\033[1;32mΣκέψη AI... (Όριο: {current_time_limit:.1f}s)\033[0m")
                nodes_counter = [0]
                start_time = time.time()
                best_score_full, best_line_full, last_best_move = 0.0, [], None
                depth = 1

                input_queue = queue.Queue()
                def input_thread(q):
                    try: input(); q.put(True)
                    except: pass
                threading.Thread(target=input_thread, args=(input_queue,), daemon=True).start()

                # Iterative Deepening
                while True:
                    if not input_queue.empty() or (time.time() - start_time >= current_time_limit):
                        break
                    
                    result = minimax(board, depth, -float('inf'), float('inf'), board.turn, nodes_counter, start_time, current_time_limit, best_move_hint=last_best_move, interrupt_queue=input_queue)
                    
                    # Η ΔΙΟΡΘΩΣΗ: Έλεγχος για None πριν το formatting
                    if result is not None and result[0] is not None:
                        best_score_full, best_line_full = result
                        last_best_move = best_line_full[0] if best_line_full else None
                        move_san = board.san(best_line_full[0]) if best_line_full else "???"
                        
                        print(f"\r\033[36m[D:{depth:2d}] Best:{move_san:5} | Eval:{best_score_full:+.2f} | Nodes:{nodes_counter[0]}\033[0m", end="", flush=True)
                        if abs(best_score_full) >= 9000: break
                    else:
                        break # Διακοπή αν η minimax επέστρεψε None
                        
                    depth += 1
                    if depth > 25: break
                
                print()
                selected_move = best_line_full[0] if best_line_full else list(board.legal_moves)[0]
                time_spent = time.time() - start_time

            # Ενημέρωση Ρολογιού AI
            if ai_side == chess_lib.WHITE: w_clock = w_clock - time_spent + inc
            else: b_clock = b_clock - time_spent + inc

            prefix = f"{move_number}." if board.turn == chess_lib.WHITE else f"{move_number}..."
            print(f"\033[1;32m{prefix} AI παίζει: {board.san(selected_move)}\033[0m")
            board.push(selected_move)
            if board.turn == chess_lib.WHITE: move_number += 1

        else:
            # --- HUMAN TURN ---
            print("\033[1;32m==> Σειρά σου!\033[0m")
            h_start = time.time()
            move_str = input("\033[36mΚίνηση (e2e4 ή 'undo'): \033[0m").strip().lower()
            h_end = time.time()
            
            time_spent = h_end - h_start
            if ai_side == chess_lib.BLACK: w_clock = w_clock - time_spent + inc
            else: b_clock = b_clock - time_spent + inc

            # Friendly Time Warning
            if (ai_side == chess_lib.BLACK and w_clock <= 0) or (ai_side == chess_lib.WHITE and b_clock <= 0):
                print("\033[31m[!] Ο χρόνος σου έληξε, αλλά συνεχίζουμε!\033[0m")
            ...
            try:
                ...
                if move in board.legal_moves:
                    board.push(move)
                    if board.turn == chess_lib.WHITE: move_number += 1
                else: print("\033[31mΜη νόμιμη κίνηση.\033[0m")
            except: print("\033[31mΛάθος μορφή (uci).\033[0m")
