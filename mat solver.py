...
def has_forced_mate(board, depth, total_depth, mode, ply, alpha_ref, beta_ref):
    global NODES, TT
    NODES += 1
    ...
    if is_or_node:  
        ...
        for mv in moves:
            ...
            board.pop()
            if not ok:
                TT[key] = (depth, False, [], mv.uci())
                return False, []
            else:
                current_line = [mv] + sub_line
                if longest_line is None or len(current_line) > len(longest_line):
                    longest_line = current_line
                    worst_move_uci = mv.uci()
                HISTORY[(side_flag, mv.from_square, mv.to_square)] -= 1
        if longest_line is not None:
            TT[key] = (depth, True, longest_line, worst_move_uci)
            return True, longest_line
        TT[key] = (depth, False, [], None)
        return False, []
...
def find_first_forced_move(fen, depth, mode, focus=None):
    ...
    if focus:
        ...
        if mv:
            if mode == "quiet" and not is_quiet(board, mv):
                ...
                board.push(mv)
                ok, sub_line = has_forced_mate(board, depth - 1, depth, mode, 1, False, True)
                board.pop()
                if ok:
    ...
    for d in range(2, depth + 1, 2):
        ...
        tt_move_uci = tt_data[3] if tt_data else None
        ...
        for mv in root_moves:
            ...
            print(f"--> Εξετάζω: {san}")
            board.push(mv)
            ok, sub_line = has_forced_mate(board, d - 1, depth, mode, 1, False, True)
            board.pop()
            if ok:
                print(f"--> Εξετάζω: {san}!  [Βρέθηκε ματ!]")
                found_here = [mv] + sub_line
                best_line = found_here
                break
        if found_here:
            return best_line
    return best_line
...
def run_solver_on_fen(fen, full_moves_mate):
    ...
    start = time.time()
    mate_line = find_first_forced_move(fen, depth, mode, focus)
    end = time.time()
    elapsed = end - start
    ...
    if mate_line:
        ...
        if temp_board.turn == chess.BLACK:
            variation_str += f"{temp_board.fullmove_number}... "
        for move in mate_line:
            if temp_board.turn == chess.WHITE:
                variation_str += f"{temp_board.fullmove_number}. {temp_board.san(move)} "
            else:
                variation_str += f"{temp_board.san(move)} "
            temp_board.push(move)
        print(f"Βαριάντα Ματ: {variation_str.strip()}")
    else:
        print("Δεν βρέθηκε υποχρεωτικό ματ.")
    print(f"Χρόνος: {elapsed:.2f} δευτερόλεπτα")
    print(f"Θέσεις που εξετάστηκαν: {NODES}")
    print(f"Ταχύτητα: {nps} θέσεις/δευτερόλεπτο")

# --- Main loop ---
while True:
    print("\n=== Don Zouán Mate Solver v3.2 — Full Variation Turbo ===")
