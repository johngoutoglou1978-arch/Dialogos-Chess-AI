...
# Ορισμοί σταθερών για το Transposition Table (TT)
TT_EXACT = 0
TT_LOWERBOUND = 1
TT_UPPERBOUND = 2

# Αρχικοποίηση TT (Περιορισμένο μέγεθος για Android / Lenovo Tablet)
transposition_table = {}
MAX_TT_SIZE = 1000000

def minimax(board, depth, alpha, beta, maximizing, nodes, start_time, time_limit, 
            ply=0, allow_null=True, best_move_hint=None, interrupt_queue=None):
    ...
    # Αποθήκευση αρχικών ορίων και δημιουργία έγκυρου Zobrist Hash
    alpha_orig = alpha
    ...
    if board_hash in transposition_table:
        ...
        if tt_entry['depth'] >= depth:
            if tt_entry['flag'] == TT_EXACT:
                return tt_entry['value'], tt_entry['line']
    ...
    # 5. --- SAFE NULL MOVE PRUNING (NMP) ---
    # Κλήση της εξωτερικής σου συνάρτησης is_endgame(board) για αποφυγή σφάλματος AttributeError
    if allow_null and depth >= 3 and not board.is_check() and not is_endgame(board):
        R = 2 if depth < 6 else 3
        board.push(chess.Move.null())
        null_score, _ = minimax(board, depth - 1 - R, -beta, -beta + 1, not maximizing, 
                                nodes, start_time, time_limit, ply=ply+1, allow_null=False, interrupt_queue=interrupt_queue)
        board.pop()
    ...
    # Προτεραιότητα 1: Κίνηση από το Transposition Table
    if tt_move and tt_move in moves:
        moves.remove(tt_move)
        moves.insert(0, tt_move)
    # Προτεραιότητα 2: Hint Κίνηση
    elif best_move_hint:
    ...
    if maximizing:
        ...
        for move in moves:
            ...
            if board.is_repetition(3):
                ...
                if first_move:
                    score, line = minimax(board, depth - 1, alpha, beta, False, nodes, start_time, time_limit, 
                                          ply=ply+1, allow_null=allow_null, interrupt_queue=interrupt_queue)
                else:
                    score, line = minimax(board, depth - 1, alpha, alpha + 1, False, nodes, start_time, time_limit, 
                                          ply=ply+1, allow_null=allow_null, interrupt_queue=interrupt_queue)
        ...
        for move in moves:
            ...
            if board.is_repetition(3):
                ...
                # 7. --- PRINCIPAL VARIATION SEARCH (PVS) ---
                if first_move:
                    score, line = minimax(board, depth - 1, alpha, beta, True, nodes, start_time, time_limit, 
                                          ply=ply+1, allow_null=allow_null, interrupt_queue=interrupt_queue)
                else:
                    score, line = minimax(board, depth - 1, beta - 1, beta, True, nodes, start_time, time_limit, 
                                          ply=ply+1, allow_null=allow_null, interrupt_queue=interrupt_queue)
    ...
    transposition_table[board_hash] = {
        ...
        'depth': depth,
        'flag': flag,
        'move': best_line[0] if best_line else None,
        'line': best_line
    }
