...
# Ο επίσημος πίνακας (ποινή σε centipawns)
...
# Βάρη Stockfish για τον υπολογισμό των Attack Units
# (Μεγαλύτερες τιμές ώστε μετά τη διαίρεση να πιάνουν σωστά τον πίνακα)
VAL_KNIGHT = 20
VAL_BISHOP = 20
VAL_ROOK   = 40
VAL_QUEEN  = 80
...
def calculate_king_safety(board, king_color):
    ...
    # -------------------------------------------------------------------------
    # 1. KING ZONE & ΕΧΘΡΙΚΕΣ ΕΠΙΘΕΣΕΙΣ
    # -------------------------------------------------------------------------
    king_zone = set(board.attacks(king_square))
    ...
    for f_offset in [-1, 0, 1]:
        ...
        if 0 <= target_file <= 7 and 0 <= target_rank <= 7:
            ...
            if 0 <= target_rank + direction <= 7:
                king_zone.add(chess.square(target_file, target_rank + direction))

    attack_units = 0
    attacking_pieces_count = 0
    
    # Έλεγχος ανά κομμάτι (όχι ανά τετράγωνο)
    for piece_type, weight in [(chess.KNIGHT, VAL_KNIGHT), 
                               (chess.BISHOP, VAL_BISHOP), 
                               (chess.ROOK, VAL_ROOK), 
                               (chess.QUEEN, VAL_QUEEN)]:
        
        pieces = board.pieces(piece_type, enemy_color)
        for piece_square in pieces:
            piece_attacks = board.attacks(piece_square)
            
            # Πόσα τετράγωνα της ζώνης ελέγχει αυτό το συγκεκριμένο κομμάτι;
            intersect_count = len(king_zone.intersection(piece_attacks))
            
            if intersect_count > 0:
                attacking_pieces_count += 1
                # Περισσότερα τετράγωνα = μεγαλύτερη ποινή
                attack_units += weight + (intersect_count * 2)

    # Υπολογισμός βασικής ποινής επίθεσης
    king_danger = 0
    if attacking_pieces_count >= 2:
        # Διαίρεση διά 8 (κλασική προσέγγιση Stockfish για scaling του πίνακα)
        index = min(attack_units // 8, 99)
        king_danger = STOCKFISH_SAFETY_TABLE[index]

    # -------------------------------------------------------------------------
    # 2. KING SHELTER & OPEN FILES (Καταφύγιο και Ανοιχτές Στήλες)
    # -------------------------------------------------------------------------
    # Ελέγχουμε τις 3 στήλες (του βασιλιά και τις διπλανές του)
    for f_offset in [-1, 0, 1]:
        file_to_check = king_file + f_offset
        if not (0 <= file_to_check <= 7):
            continue
            
        # Δημιουργία bitmask για όλη τη στήλη
        file_mask = chess.BB_FILES[file_to_check]
        
        own_pawns = board.pieces(chess.PAWN, king_color) & file_mask
        enemy_pawns = board.pieces(chess.PAWN, enemy_color) & file_mask
        
        # Περίπτωση Α: Πλήρως ανοιχτή στήλη (Open File) -> Μεγάλος κίνδυνος
        if not own_pawns and not enemy_pawns:
            king_danger += 25
        # Περίπτωση Β: Ημι-ανοιχτή στήλη για τον εχθρό (Half-Open) -> Μεσαίος κίνδυνος
        elif not own_pawns and enemy_pawns:
            king_danger += 15
            
        # Περίπτωση Γ: Το δικό μας πιόνι έχει προχωρήσει πολύ μπροστά (κακό καταφύγιο)
        elif own_pawns:
            # Βρίσκουμε το rank του δικού μας πιονιού σε αυτή τη στήλη
            pawn_squares = list(own_pawns)
            # Αν υπάρχουν διπλά, παίρνουμε το πιο κοντινό στον βασιλιά
            pawn_rank = min(chess.square_rank(sq) for sq in pawn_squares) if king_color == chess.WHITE else max(chess.square_rank(sq) for sq in pawn_squares)
            
            # Αν έχει απομακρυνθεί από τη βασική του θέση (rank 2 για λευκά, 7 για μαύρα)
            distance = abs(pawn_rank - (1 if king_color == chess.WHITE else 6))
            king_danger += distance * 8
    ...
    if maximizing:
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if board.is_repetition(3):
                ...
                if moves_searched == 0:
                    # Η πρώτη κίνηση (PV node) ψάχνεται πάντα με πλήρες παράθυρο
                    ...
                    # Για τις επόμενες κινήσεις, ελέγχουμε αν μπορούμε να κάνουμε LMR
                    ...
                    # 1. Ψάχνουμε με Μηδενικό Παράθυρο (Null Window Search: [alpha, alpha + 1])
                    score, line = minimax(board, search_depth, alpha, alpha + 1, False, nodes, start_time, time_limit, ply=ply+1, interrupt_queue=interrupt_queue)
                    
                    # 2. Αν η κίνηση απέτυχε προς τα πάνω (score > alpha) και είχαμε κάνει LMR, ξαναψάχνουμε χωρίς μείωση
                    if score is not None and score > alpha and search_depth < (depth - 1 + extension):
                    ...
                    # 3. Αν το σκορ παραμένει καλύτερο από το alpha, κάνουμε πλήρη αναζήτηση με κανονικό παράθυρο
        ...
        if tt_entry is None or depth >= tt_entry['depth']:
            if len(transposition_table) >= MAX_TT_SIZE:
                # Καθαρισμός Transposition Table αν γεμίσει
    ...
    # --- MINIMIZING PLAYER (Μαύρα) ---
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if board.is_repetition(3):
                ...
                # --- PRINCIPAL VARIATION SEARCH (PVS) & LMR ---
                if moves_searched == 0:
                    # Η πρώτη κίνηση (PV node) ψάχνεται πάντα με πλήρες παράθυρο [alpha, beta]
                    ...
                    # 1. Ψάχνουμε με Μηδενικό Παράθυρο για τα Μαύρα (Null Window Search: [beta - 1, beta])
                    score, line = minimax(board, search_depth, beta - 1, beta, True, nodes, start_time, time_limit, ply=ply+1, interrupt_queue=interrupt_queue)
                    
                    # 2. Αν η κίνηση απέτυχε προς τα κάτω (score < beta) και είχαμε κάνει LMR, ξαναψάχνουμε χωρίς μείωση
                    if score is not None and score < beta and search_depth < (depth - 1 + extension):
                    ...
                    # 3. Αν το σκορ παραμένει καλύτερο για τα μαύρα, κάνουμε πλήρη αναζήτηση με κανονικό παράθυρο
