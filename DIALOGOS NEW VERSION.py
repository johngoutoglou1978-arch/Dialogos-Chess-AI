...
def evaluate_board(board):
    ...
    # ==================================================
    # ROOK STRATEGY (UPDATED: CONTESTING & SEMI-OPEN BATTERY)
    # ==================================================
    # Προ-υπολογισμός των στηλών που έχουν μαύρους και λευκούς πύργους
    w_rook_files = [chess.square_file(sq) for sq in chess.SquareSet(w_rooks)]
    b_rook_files = [chess.square_file(sq) for sq in chess.SquareSet(b_rooks)]
    ...
    for r_sq in chess.SquareSet(w_rooks):
        ...
        if r_idx == 6: score += 25
        if f_idx in (3, 4): score += 20  
        
        if not (all_pawns & f_mask): score += 30  
        elif not (w_pawns & f_mask): score += 15

        # 1. ΔΙΕΚΔΙΚΗΣΗ ΣΤΗΛΗΣ: Αν ο Μαύρος έχει πύργο εδώ, ο Λευκός πάει απέναντι
        if f_idx in b_rook_files:
            score += 20  

        # 2. ΔΙΠΛΩΜΑ ΣΕ ΑΝΟΙΧΤΗ/ΗΜΙΑΝΟΙΧΤΗ ΣΤΗΛΗ: Αν δεν υπάρχει δικό μας πιόνι ΚΑΙ υπάρχουν 2 πύργοι στη στήλη
        if not (w_pawns & f_mask) and w_rook_files.count(f_idx) >= 2:
            score += 15  # Έξτρα μπόνους διπλώματος (ανά πύργο, άρα +30 συνολικά για το ζευγάρι!)
    ...
    for r_sq in chess.SquareSet(b_rooks):
        ...
        if not (all_pawns & f_mask): score -= 30  
        elif not (b_pawns & f_mask): score -= 15

        # 1. ΔΙΕΚΔΙΚΗΣΗ ΣΤΗΛΗΣ: Αν ο Λευκός έχει πύργο εδώ, ο Μαύρος πάει απέναντι
        if f_idx in w_rook_files:
            score -= 20  

        # 2. ΔΙΠΛΩΜΑ ΣΕ ΑΝΟΙΧΤΗ/ΗΜΙΑΝΟΙΧΤΗ ΣΤΗΛΗ: Αν δεν υπάρχει δικό μας πιόνι ΚΑΙ υπάρχουν 2 πύργοι στη στήλη
        if not (b_pawns & f_mask) and b_rook_files.count(f_idx) >= 2:
            score -= 15  # Έξτρα μπόνους διπλώματος για τα μαύρα
