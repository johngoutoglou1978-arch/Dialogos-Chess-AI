...
def minimax(board, depth, alpha, beta, maximizing, nodes, start_time, time_limit, ply=0, best_move_hint=None, interrupt_queue=None):
    ...
    # 5. --- MOVE ORDERING (STOCKFISH LOGIC) ---
    scored_moves = []
    current_killers = killer_moves[ply] if ply < len(killer_moves) else None

    # Χρήση generate_legal_moves για ταχύτητα μνήμης
    for move in board.generate_legal_moves():
        # Χρησιμοποιούμε τη νέα mvv_lva περνώντας της tt_move και killers
        score_val = mvv_lva(move, board, tt_move=tt_move, killers=current_killers)
        scored_moves.append((score_val, move))
    
    # Ταξινόμηση - Η tt_move και τα captures μπαίνουν αυτόματα πρώτα λόγω σκορ
    scored_moves.sort(key=lambda x: x[0], reverse=True)
    moves = [item[1] for item in scored_moves]

    # Διαχειριζόμαστε μόνο το hint αν υπάρχει (καθώς το tt_move έχει ήδη μπει πρώτο)
    if best_move_hint:
    ...
    if maximizing:
        ...
        for moves_searched, move in enumerate(moves):
            ...
            # 🌟 LATE MOVE PRUNING
            if depth <= 4 and not in_check and not is_capture and not is_promo:
        ...
        if board.turn == ai_side:
            ...
            def live_timer():
                ...
                while not stop_timer:
                    time.sleep(1)
                    if not stop_timer and seconds % 60 == 0:
