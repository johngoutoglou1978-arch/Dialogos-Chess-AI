...
int alphabeta(int alpha, int beta, int depth, chess::Board& board, int ply, chess::Move& best_move) {
    ...
    for (size_t i = 0; i < moves.size(); i++) {
        // Έλεγχος διακοπής στην αρχή του loop κινήσεων
        if (ai_interrupt_flag.load()) return alpha;
        ...
        // Τοπική μεταβλητή για να μην καταστρέφεται η best_move της ρίζας από τα βαθύτερα ply
        ...
        if (search_pv) {
            ...
            if (depth >= 3 && legal_moves_searched > 3 && !board.inCheck() && !is_capture && !is_promotion) {
                lmr_reduction = 1 + (legal_moves_searched / 8); 
            ...
            score = -alphabeta(-(alpha + 1), -alpha, depth - 1 - lmr_reduction, board, ply + 1, sub_best_move);
            
            if (score > alpha && lmr_reduction > 0) {
                score = -alphabeta(-(alpha + 1), -alpha, depth - 1, board, ply + 1, sub_best_move);
            }
        ...
        // Έλεγχος διακοπής αμέσως μετά την επιστροφή από την αναζήτηση
        if (ai_interrupt_flag.load()) return alpha;
        ...
        if (score >= beta) {
            ...
            // Αποθήκευση στη ρίζα ΜΟΝΟ αν δεν έχουμε διακοπή
            if (ply == 0 && !ai_interrupt_flag.load()) best_move = mv; 
            return beta; 
        }
    ...
    while (true) {
        ...
        if (board.sideToMove() == ai_side) {
            ...
            while (true) {
                ...
                // 1. Έλεγχος χρόνου στην αρχή του βάθους
                ...
                // 2. ΚΡΙΣΙΜΟ: Αν ο χρόνος τελείωσε κατά τη διάρκεια της alphabeta, ΑΓΝΟΟΥΜΕ το αποτέλεσμα
                if (ai_interrupt_flag) {
                    break; 
                }
                ...
                if (depth > 1 && (score <= alpha || score >= beta)) {
                    ...
                    if (ai_interrupt_flag) break; // Αγνοούμε και εδώ αν διακόπηκε
                }

                // 3. Αποθήκευση ΜΟΝΟ αν το βάθος ολοκληρώθηκε επιτυχώς και έγκυρα
                if (!ai_interrupt_flag && current_best != Move::NO_MOVE) {
            ...
            // Εξασφάλιση τερματισμού του thread (αν το AI σταμάτησε από χρόνο, στέλνουμε ένα εικονικό enter στην κονσόλα αν χρειάζεται, ή απλά κάνουμε join αν πατήθηκε)
            if (ai_interrupt_flag) {
                // Σημείωση: Το input_thread ίσως παραμείνει μπλοκαρισμένο στο getline αν βγήκαμε από χρόνο. 
                // Για console εφαρμογές, η απλή λύση είναι το detach ώστε να μην κρατάει τη ροή μας.
                input_thread.detach(); 
            } else {
