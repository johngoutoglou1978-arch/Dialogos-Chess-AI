...
def evaluate_board(board):
    ...
    # --- 4. STOCKFISH-STYLE COMPREHENSIVE KING SAFETY WITH ATTACKERS ---
    if endgame_weight < 1.0:
        king_safety_penalty = 0

        # ΑΣΦΑΛΕΙΑ ΛΕΥΚΟΥ ΒΑΣΙΛΙΑ
        if wk is not None:
            ...
            if wk_rank <= 2:
                ...
                shield_close = 0
                shield_far = 0
                for f in range(max(0, wk_file - 1), min(8, wk_file + 2)):
                    if wk_rank + 1 < 8: shield_close |= chess.BB_SQUARES[chess.square(f, wk_rank + 1)]
                    if wk_rank + 2 < 8: shield_far |= chess.BB_SQUARES[chess.square(f, wk_rank + 2)]

                pawns_close = int(w_pawns & shield_close).bit_count()
                pawns_far = int(w_pawns & shield_far).bit_count()
                missing_pawns = 3 - (pawns_close + pawns_far)
                if missing_pawns > 0:
                    king_safety_penalty += missing_pawns * 30

                # Β) Ανοιχτές Στήλες
                file_mask = chess.BB_FILES[wk_file]
                if not (w_pawns & file_mask):
                    king_safety_penalty += 25
                    if not (b_pawns & file_mask):
                        king_safety_penalty += 20

                # Γ) ΖΩΝΗ ΤΡΟΜΟΚΡΑΤΙΑΣ (KING ATTACKERS)
                # Παίρνουμε τα 8 γειτονικά τετράγωνα γύρω από τον βασιλιά
                king_zone_attackers = set()
                for sq in chess.SquareSet(chess.BB_KING_ATTACKS[wk]):
                    # Βρίσκουμε ποια ΜΑΥΡΑ κομμάτια επιτίθενται σε αυτά τα τετράγωνα
                    # Αγνοούμε τα μαύρα πιόνια, μετράμε μόνο μεγάλα κομμάτια (Ίππους, Αξιωματικούς, Πύργους, Βασίλισσες)
                    for attacker_sq in board.attackers(chess.BLACK, sq):
                        ...
                        if piece_type != chess.PAWN and piece_type != chess.KING:
                            king_zone_attackers.add(attacker_sq)
                
                # Εκθετικό πέναλτι ανάλογα με το πλήθος των επιτιθέμενων
                num_attackers = len(king_zone_attackers)
                if num_attackers > 0:
                    king_safety_penalty += (num_attackers ** 2) * 15

        # ΑΣΦΑΛΕΙΑ ΜΑΥΡΟΥ ΒΑΣΙΛΙΑ
        if bk is not None:
            ...
            if bk_rank >= 5:
                ...
                shield_close = 0
                shield_far = 0
                for f in range(max(0, bk_file - 1), min(8, bk_file + 2)):
                    if bk_rank - 1 >= 0: shield_close |= chess.BB_SQUARES[chess.square(f, bk_rank - 1)]
                    if bk_rank - 2 >= 0: shield_far |= chess.BB_SQUARES[chess.square(f, bk_rank - 2)]

                pawns_close = int(b_pawns & shield_close).bit_count()
                pawns_far = int(b_pawns & shield_far).bit_count()
                missing_pawns = 3 - (pawns_close + pawns_far)
                if missing_pawns > 0:
                    king_safety_penalty -= missing_pawns * 30

                # Β) Ανοιχτές Στήλες
                file_mask = chess.BB_FILES[bk_file]
                if not (b_pawns & file_mask):
                    king_safety_penalty -= 25
                    if not (w_pawns & file_mask):
                        king_safety_penalty -= 20
                ...
                for sq in chess.SquareSet(chess.BB_KING_ATTACKS[bk]):
                    # Βρίσκουμε ποια ΛΕΥΚΑ κομμάτια επιτίθενται στη ζώνη του μαύρου βασιλιά
        ...
        # Εφαρμογή των συνολικών ποινών προστασίας μειούμενη προς το Endgame
        score -= int(king_safety_penalty * (1 - endgame_weight))
