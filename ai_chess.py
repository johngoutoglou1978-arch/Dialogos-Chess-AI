...
def evaluate_board(board):
    # Πίνακας ασφαλείας Stockfish (Inline για αποφυγή NameError)
    STOCKFISH_SAFETY_TABLE = [
           0,    0,  120,  240,  360,  600,  840, 1080, 1440, 1800,
        2160, 2640, 3120, 3600, 4200, 4680, 5280, 6000, 6720, 7440,
        8160, 9000, 9840,10200,10680,11640,12600,13560,14640,15720,
       16800,18000,20280,21600,22920,24240,25560,27000,28440,29760,
       31200,32640,33960,35400,36840,38280,39600,41040,42480,43920,
       45240,46680,48120,49440,50880,52320,53760,55080,56520,57960,
       59280,60000,60000,60000,60000,60000,60000,60000,60000,60000,
    ...
    for sq in chess.SquareSet(w_pawns): score += PAWN_TABLE[sq ^ 56]
    for sq in chess.SquareSet(b_pawns): score -= PAWN_TABLE[sq]
    for sq in chess.SquareSet(w_knights): score += KNIGHT_TABLE[sq ^ 56]
    for sq in chess.SquareSet(b_knights): score -= KNIGHT_TABLE[sq]
    for sq in chess.SquareSet(w_bishops): score += BISHOP_TABLE[sq ^ 56]
    for sq in chess.SquareSet(b_bishops): score -= BISHOP_TABLE[sq]
    for sq in chess.SquareSet(w_rooks): score += ROOK_TABLE[sq ^ 56]
    for sq in chess.SquareSet(b_rooks): score -= ROOK_TABLE[sq]
    for sq in chess.SquareSet(w_queens): score += QUEEN_TABLE[sq ^ 56]
    for sq in chess.SquareSet(b_queens): score -= QUEEN_TABLE[sq]

    # 6. Βασιλιάς - Δυναμική εναλλαγή θέσης (Middlegame / Endgame)
    wk = board.king(chess.WHITE)
    bk = board.king(chess.BLACK)

    if wk is not None:
        score += int((1 - endgame_weight) * KING_TABLE[wk ^ 56] + endgame_weight * KING_ENDGAME_TABLE[wk ^ 56])
    if bk is not None:
        score -= int((1 - endgame_weight) * KING_TABLE[bk] + endgame_weight * KING_ENDGAME_TABLE[bk])

    # --- ΕΝΣΩΜΑΤΩΜΕΝΗ ΓΝΩΣΗ: ΑΣΦΑΛΕΙΑ ΒΑΣΙΛΙΑ (INLINE KING SAFETY) ---
    if endgame_weight < 1.0:
        safety_scale = 1.0 - endgame_weight
        
        # Α) Για τον Λευκό Βασιλιά (wk)
        if wk is not None:
            w_zone = chess.BB_KING_ATTACKS[wk] | chess.BB_SQUARES[wk]
            w_rank = chess.square_rank(wk)
            w_file = chess.square_file(wk)
            if w_rank < 7:  # Προσθήκη 3 μπροστινών τετραγώνων
                for f in range(max(0, w_file - 1), min(8, w_file + 2)):
                    w_zone |= chess.BB_SQUARES[chess.square(f, w_rank + 1)]
            
            w_units = 0
            w_attackers = 0
            for sq in chess.SquareSet(b_knights):
                if chess.BB_KNIGHT_ATTACKS[sq] & w_zone: w_units += 2; w_attackers += 1
            for sq in chess.SquareSet(b_bishops):
                if chess.ray(sq, wk) & w_zone: w_units += 2; w_attackers += 1
            for sq in chess.SquareSet(b_rooks):
                if chess.ray(sq, wk) & w_zone: w_units += 3; w_attackers += 1
            for sq in chess.SquareSet(b_queens):
                if chess.ray(sq, wk) & w_zone: w_units += 5; w_attackers += 1
                
            if w_attackers > 1:  # Μόνο αν επιτίθενται τουλάχιστον 2 κομμάτια
                if board.is_check() and board.turn == chess.WHITE:
                    if board.attackers(chess.BLACK, wk) & board.queens: w_units += 6
                    elif board.attackers(chess.BLACK, wk) & board.rooks: w_units += 4
                w_units = min(w_units, 99)
                score -= int((STOCKFISH_SAFETY_TABLE[w_units] // 2) * safety_scale)

        # Β) Για τον Μαύρο Βασιλιά (bk)
        if bk is not None:
            b_zone = chess.BB_KING_ATTACKS[bk] | chess.BB_SQUARES[bk]
            b_rank = chess.square_rank(bk)
            b_file = chess.square_file(bk)
            if b_rank > 0:  # Προσθήκη 3 μπροστινών τετραγώνων
                for f in range(max(0, b_file - 1), min(8, b_file + 2)):
                    b_zone |= chess.BB_SQUARES[chess.square(f, b_rank - 1)]
            
            b_units = 0
            b_attackers = 0
            for sq in chess.SquareSet(w_knights):
                if chess.BB_KNIGHT_ATTACKS[sq] & b_zone: b_units += 2; b_attackers += 1
            for sq in chess.SquareSet(w_bishops):
                if chess.ray(sq, bk) & b_zone: b_units += 2; b_attackers += 1
            for sq in chess.SquareSet(w_rooks):
                if chess.ray(sq, bk) & b_zone: b_units += 3; b_attackers += 1
            for sq in chess.SquareSet(w_queens):
                if chess.ray(sq, bk) & b_zone: b_units += 5; b_attackers += 1
                
            if b_attackers > 1:  # Μόνο αν επιτίθενται τουλάχιστον 2 κομμάτια
                if board.is_check() and board.turn == chess.BLACK:
                    if board.attackers(chess.WHITE, bk) & board.queens: b_units += 6
                    elif board.attackers(chess.WHITE, bk) & board.rooks: b_units += 4
                b_units = min(b_units, 99)
                score += int((STOCKFISH_SAFETY_TABLE[b_units] // 2) * safety_scale)

    # 4. Tempo Bonus
    score += (15 if board.turn == chess.WHITE else -15)

    # --- ΜΠΟΝΟΥΣ ΑΠΟΤΡΟΠΗΣ ΚΑΡΦΩΜΑΤΟΣ / ΑΠΕΙΛΗΣ (PROPHYLAXIS BONUS) ---
    PROPHYLAXIS_BONUS = 15
    if board.piece_at(chess.H3) == chess.Piece(chess.PAWN, chess.WHITE) and board.is_attacked_by(chess.BLACK, chess.G4):
        score += PROPHYLAXIS_BONUS
    if board.piece_at(chess.A3) == chess.Piece(chess.PAWN, chess.WHITE) and board.is_attacked_by(chess.BLACK, chess.B4):
        score += PROPHYLAXIS_BONUS
    if board.piece_at(chess.H6) == chess.Piece(chess.PAWN, chess.BLACK) and board.is_attacked_by(chess.WHITE, chess.G5):
        score -= PROPHYLAXIS_BONUS
    if board.piece_at(chess.A6) == chess.Piece(chess.PAWN, chess.BLACK) and board.is_attacked_by(chess.WHITE, chess.B5):
        score -= PROPHYLAXIS_BONUS

    # --- ΜΠΟΝΟΥΣ ΔΙΩΞΙΜΑΤΟΣ ΑΞΙΩΜΑΤΙΚΟΥ ΜΕ ΥΠΟΣΤΗΡΙΞΗ (SUPPORTED CHASING) ---
    CHASING_BONUS = 25
    if board.piece_at(chess.G4) == chess.Piece(chess.PAWN, chess.WHITE) and board.piece_at(chess.H3) == chess.Piece(chess.PAWN, chess.WHITE) and board.piece_at(chess.H5) == chess.Piece(chess.BISHOP, chess.BLACK):
        score += CHASING_BONUS
    if board.piece_at(chess.B4) == chess.Piece(chess.PAWN, chess.WHITE) and board.piece_at(chess.A3) == chess.Piece(chess.PAWN, chess.WHITE) and board.piece_at(chess.A5) == chess.Piece(chess.BISHOP, chess.BLACK):
        score += CHASING_BONUS
    if board.piece_at(chess.G5) == chess.Piece(chess.PAWN, chess.BLACK) and board.piece_at(chess.H6) == chess.Piece(chess.PAWN, chess.BLACK) and board.piece_at(chess.H4) == chess.Piece(chess.BISHOP, chess.WHITE):
        score -= CHASING_BONUS
    if board.piece_at(chess.B5) == chess.Piece(chess.PAWN, chess.BLACK) and board.piece_at(chess.A6) == chess.Piece(chess.PAWN, chess.BLACK) and board.piece_at(chess.A4) == chess.Piece(chess.BISHOP, chess.WHITE):
        score -= CHASING_BONUS
...
def mvv_lva(move, board):
    """
    Βελτιωμένο Move Ordering βασισμένο σε:
    1) Κοψίματα (Captures) - MVV-LVA
    2) Στρατηγική Θέσης Sunfish (PST Positioning) - Χωρίς push/pop
    """
    score = 0
    ...
    if board.is_capture(move):
        ...
        # Προσθέτουμε +100000 για να μπουν όλα τα captures στην κορυφή της λίστας
        score += 100000 + (piece_values[victim_type] * 10) - piece_values[attacker.piece_type]

    # 2️⃣ Η ΕΞΥΠΝΑΔΑ ΤΟΥ SUNFISH (PST Bonus)
    # Υπολογίζουμε τη διαφορά αξίας μεταξύ του τετραγώνου που πάει και αυτού που φεύγει
    from_sq = move.from_square
    to_sq = move.to_square

    # Αν παίζουν τα λευκά, η python-chess χρειάζεται XOR 56 (sq ^ 56) όπως ακριβώς στην evaluate_board σας
    if board.turn == chess.WHITE:
        from_sq ^= 56
        to_sq ^= 56

    # Αντιστοίχιση του κομματιού με τον κατάλληλο πίνακα Sunfish που έχετε ορίσει
    if attacker.piece_type == chess.PAWN:
        score += PAWN_TABLE[to_sq] - PAWN_TABLE[from_sq]
    elif attacker.piece_type == chess.KNIGHT:
        score += KNIGHT_TABLE[to_sq] - KNIGHT_TABLE[from_sq]
    elif attacker.piece_type == chess.BISHOP:
        score += BISHOP_TABLE[to_sq] - BISHOP_TABLE[from_sq]
    elif attacker.piece_type == chess.ROOK:
        score += ROOK_TABLE[to_sq] - ROOK_TABLE[from_sq]
    elif attacker.piece_type == chess.QUEEN:
        score += QUEEN_TABLE[to_sq] - QUEEN_TABLE[from_sq]
    elif attacker.piece_type == chess.KING:
        # Χρησιμοποιούμε τον βασικό πίνακα Middlegame για απλότητα στο ordering
        score += KING_TABLE[to_sq] - KING_TABLE[from_sq]

    # 3️⃣ ΠΡΟΑΓΩΓΗ ΠΙΟΝΙΟΥ
    if move.promotion:
        score += 90000
...
def score_move(move, board, ply):
    ...
    # 2. Killer moves (Απευθείας έλεγχος με index αντί για [0] και)
    # Υποθέτοντας ότι το killer_moves[ply] περιέχει μια λίστα ή πλειάδα με 2 κινήσεις
    ...
    if piece:
        ...
        score += history_heuristic.get((piece.piece_type, move.to_square), 0)
...
def minimax(board, depth, alpha, beta, maximizing, nodes, start_time, time_limit, ply=0, best_move_hint=None, interrupt_queue=None):
    ...
    # 6. --- ΣΤΑΤΙΚΗ ΑΞΙΟΛΟΓΗΣΗ ΚΑΙ PRUNINGS ---
    ...
    if maximizing:
        ...
        if depth <= 3 and not in_check and not is_prev_move_null:
            rfp_margin = depth * 150
            if static_eval - rfp_margin >= beta:
                return beta, []

        # 🌟 RAZORING
        if depth == 1 and not in_check and static_eval + 300 <= alpha:
            r_score = quiescence(board, alpha, beta, maximizing, start_time, time_limit, ply=ply)
            if r_score <= alpha:
                return r_score, []
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if depth <= 3 and not in_check and not is_capture and not is_promo:
                max_moves_to_search = 3 + (depth * depth)
                if moves_searched >= max_moves_to_search:
                    break

            # --- FUTILITY PRUNING ---
            if depth <= 3 and not in_check and not is_capture and not is_promo:
                futility_margin = depth * 120
            ...
            if board.is_repetition(3):
                ...
                score = -25 if score > 1.0 else 0
                line = []
            else:
                extension = 1 if board.is_check() and ply < 30 else 0

                # 🌟 PVS & AGGRESSIVE LMR (ΛΕΥΚΑ)
                if moves_searched == 0:
                    ...
                    # Έρευνα με Μηδενικό Παράθυρο (alpha, alpha + 1)
                    score, line = minimax(board, depth - 1 - reduction, alpha, alpha + 1, False, nodes, start_time, time_limit, ply=ply+1, interrupt_queue=interrupt_queue)
                    
                    if score is not None and score > alpha and reduction > 0:
                        score, line = minimax(board, depth - 1 + extension, alpha, alpha + 1, False, nodes, start_time, time_limit, ply=ply+1, interrupt_queue=interrupt_queue)
    ...
    # =========================================================================
        ...
        # 🌟 REVERSE FUTILITY PRUNING (Καθρεφτισμένο)
        if depth <= 3 and not in_check and not is_prev_move_null:
            rfp_margin = depth * 150
            if static_eval + rfp_margin <= alpha:
                return alpha, []

        # 🌟 RAZORING (Καθρεφτισμένο)
        if depth == 1 and not in_check and static_eval - 300 >= beta:
            r_score = quiescence(board, alpha, beta, maximizing, start_time, time_limit, ply=ply)
            if r_score >= beta:
                return r_score, []
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if board.is_repetition(3):
                ...
                if moves_searched == 0:
                    ...
                    # Έρευνα με Μηδενικό Παράθυρο (beta - 1, beta) για τα Μαύρα
                    score, line = minimax(board, depth - 1 - reduction, beta - 1, beta, True, nodes, start_time, time_limit, ply=ply+1, interrupt_queue=interrupt_queue)
                    
                    if score is not None and score < beta and reduction > 0:
                        score, line = minimax(board, depth - 1 + extension, beta - 1, beta, True, nodes, start_time, time_limit, ply=ply+1, interrupt_queue=interrupt_queue)
        ...
        if best >= original_beta: flag = TT_BETA
        elif best <= original_alpha: flag = TT_ALPHA
        else: flag = TT_EXACT
