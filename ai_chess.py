...
def evaluate_board(board):
    ...
    # 2. Προσδιορισμός Endgame & Φάσης Παιχνιδιού (Game Phase για Tapered Eval)
    ...
    # Υπολογισμός phase (24 = πλήρες μέσο, 0 = καθαρό φινάλε)
    phase = (w_n_count + b_n_count + w_b_count + b_b_count) * 1 + (w_r_count + b_r_count) * 2 + (w_q_count + b_q_count) * 4
    if phase > 24: phase = 24

    # ==================================================
    # ΑΞΙΟΛΟΓΗΣΗ ΠΥΡΓΩΝ (ΕΝΙΣΧΥΜΕΝΗ ΣΤΡΑΤΗΓΙΚΗ)
    # ==================================================
    w_rk_squares = list(chess.SquareSet(w_rooks))
    b_rk_squares = list(chess.SquareSet(b_rooks))

    # --- ΛΕΥΚΟΙ ΠΥΡΓΟΙ ---
    for sq in w_rk_squares:
        f = chess.square_file(sq)
        r = chess.square_rank(sq)
        
        # 1. Έλεγχος 7ης γραμμής
        if r == 6:
            score += 20
            
        # 2. Μπόνους για κεντρικές στήλες ανάπτυξης (c1, d1, e1)
        if sq in [chess.C1, chess.D1, chess.E1]:
            score += 12  # Ενθαρρύνει την τοποθέτηση πίσω από τα κεντρικά πιόνια

        # Έλεγχος πιονιών στη στήλη
        w_pawns_in_file = chess.SquareSet(w_pawns) & chess.BB_FILES[f]
        b_pawns_in_file = chess.SquareSet(b_pawns) & chess.BB_FILES[f]
        
        # 3. Ανοιχτές και Ημιανοιχτές στήλες
        is_open_file = not w_pawns_in_file and not b_pawns_in_file
        if not w_pawns_in_file:
            if not b_pawns_in_file:
                score += 15  # Πλήρως ανοιχτή στήλη
            else:
                score += 10  # Ημιανοιχτή στήλη

        # 4. Αμφισβήτηση στήλης (Contesting the file)
        # Αν η στήλη είναι ανοιχτή και υπάρχει μαύρος πύργος σε αυτήν
        if is_open_file:
            b_rooks_in_file = chess.SquareSet(b_rooks) & chess.BB_FILES[f]
            if b_rooks_in_file:
                # Ελέγχουμε αν το τετράγωνό μας υποστηρίζεται από τουλάχιστον ένα λευκό κομμάτι
                if board.attackers(chess.WHITE, sq):
                    score += 15  # Επιπλέον μπόνους γιατί αμφισβητεί τη στήλη με ασφάλεια

    # Κάθετα διπλωμένοι Λευκοί Πύργοι
    if w_r_count == 2:
        if chess.square_file(w_rk_squares[0]) == chess.square_file(w_rk_squares[1]):
            score += 25  

    # --- ΜΑΥΡΟΙ ΠΥΡΓΟΙ ---
    for sq in b_rk_squares:
        f = chess.square_file(sq)
        r = chess.square_rank(sq)
        
        # 1. Έλεγχος 2ης γραμμής
        if r == 1:
            score -= 20
            
        # 2. Μπόνους για κεντρικές στήλες ανάπτυξης (c8, d8, e8)
        if sq in [chess.C8, chess.D8, chess.E8]:
            score -= 12
        ...
        # 4. Αμφισβήτηση στήλης (Contesting the file)
        # Αν η στήλη είναι ανοιχτή και υπάρχει λευκός πύργος σε αυτήν
        if is_open_file:
            w_rooks_in_file = chess.SquareSet(w_rooks) & chess.BB_FILES[f]
            if w_rooks_in_file:
                # Ελέγχουμε αν το τετράγωνό μας υποστηρίζεται από τουλάχιστον ένα μαύρο κομμάτι
                if board.attackers(chess.BLACK, sq):
                    score -= 15  # Επιπλέον μπόνους αμφισβήτησης για τα μαύρα
                    
    # Κάθετα διπλωμένοι Μαύροι Πύργοι
    if b_r_count == 2:
        if chess.square_file(b_rk_squares[0]) == chess.square_file(b_rk_squares[1]):
            score -= 25
    ...
    # 1. Βασιλιάς (King) - Χρήση Tapered Evaluation για ομαλή μετάβαση
    wk = board.king(chess.WHITE)
    bk = board.king(chess.BLACK)

    if wk is not None:
        wk_mirror = chess.square_mirror(wk)
        k_mg = KING_MID_TABLE[wk_mirror]
        k_eg = KING_END_TABLE[wk_mirror]
        score += ((phase * k_mg) + ((24 - phase) * k_eg)) // 24

    if bk is not None:
        k_mg = KING_MID_TABLE[bk]
        k_eg = KING_END_TABLE[bk]
        score -= ((phase * k_mg) + ((24 - phase) * k_eg)) // 24
    ...
    # 4. Πύργοι (Rooks) - ΝΕΟ κομμάτι με Tapered Evaluation
    for sq in chess.SquareSet(w_rooks):
        sq_mirror = chess.square_mirror(sq)
        r_mg = ROOK_MG[sq_mirror]
        r_eg = ROOK_EG[sq_mirror]
        score += ((phase * r_mg) + ((24 - phase) * r_eg)) // 24
    ...
    for p in chess.SquareSet(w_pawns):
        ...
        # Advance bonus (πιο ήπιο)
        score += rank * 2
        if rank in (5, 6): score += 4  # Bonus για προχωρημένο πιόνι
        if rank == 1: score += 1       # Μικρό bonus που δεν κουνήθηκε

        # Απειλές (Tactical) - Μειώθηκε στο 3 ανά κομμάτι
        enemy_attacked = board.attacks(p) & board.occupied_co[chess.BLACK]
        if enemy_attacked: 
            score += int(enemy_attacked).bit_count() * 3

        # Ποινές Δομής (Doubled / Isolated)
        if w_p_files.count(file) > 1: 
            score -= 8  # Διπλωμένο πιόνι
            
        # Διόρθωση Isolated για όλες τις στήλες (μαζί με Α και Η)
        is_isolated = True
        if file > 0 and (file - 1) in w_p_files: is_isolated = False
        if file < 7 and (file + 1) in w_p_files: is_isolated = False
        if is_isolated: 
            score -= 10  # Απομονωμένο πιόνι

        # Έλεγχος Κέντρου στο Middlegame (Μόνο για προχωρημένα D3, D4, E3, E4)
        if not endgame and p in (chess.E3, chess.E4, chess.D3, chess.D4):
            score += 10
            
        # Endgame Πασέ Πιόνι (Ήπια κλιμάκωση για να μην "τυφλώνεται")
        if endgame:
            score += rank * 4
    ...
    for p in chess.SquareSet(b_pawns):
        ...
        # Ποινές Δομής
        if b_p_files.count(file) > 1: 
            score += 8
            
        # Διόρθωση Isolated
        is_isolated = True
        if file > 0 and (file - 1) in b_p_files: is_isolated = False
        if file < 7 and (file + 1) in b_p_files: is_isolated = False
        if is_isolated: 
            score += 10

        # Έλεγχος Κέντρου
        if not endgame and p in (chess.E6, chess.E5, chess.D6, chess.D5):
            score -= 10
    ...
    if maximizing:
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if board.is_repetition(3):
                ...
                # --- AGGRESSIVE LMR ---
                if moves_searched >= 2 and depth >= 3 and not in_check and not is_capture and not is_promo:
