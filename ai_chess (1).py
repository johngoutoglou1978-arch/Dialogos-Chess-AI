...
def evaluate_board(board):
    ...
    if wk is not None:
        score += KING_END_TABLE[wk] if endgame else KING_MID_TABLE[wk]

    if bk is not None:
        mirrored = chess.square_mirror(bk)
        score -= KING_END_TABLE[mirrored] if endgame else KING_MID_TABLE[mirrored]
    ...
    if wk is not None:
        w_penalty = 0
        if board.attackers(chess.BLACK, wk): w_penalty += 50
    ...
    if bk is not None:
        b_penalty = 0
        if board.attackers(chess.WHITE, bk): b_penalty += 50
...
def minimax(board, depth, alpha, beta, maximizing, nodes, start_time, time_limit, ply=0, best_move_hint=None, interrupt_queue=None):
    ...
    # 2. --- COUNT NODES ---
    nodes[0] += 1  # Σωστή δομή λίστας όπως η αρχική σας
    ...
    # 4. --- MOVE ORDERING (ΒΕΛΤΙΣΤΟΠΟΙΗΜΕΝΟ ΧΩΡΙΣ LAMBDA OVERHEAD) ---
    scored_moves = []
    for move in board.legal_moves:
        score_val = score_move(move, board, ply)
        scored_moves.append((score_val, move))
    
    # Ταξινόμηση των tuples (τα μεγαλύτερα scores πάνε πρώτα)
    scored_moves.sort(key=lambda x: x[0], reverse=True)
    # Κρατάμε μόνο το αντικείμενο Move
    moves = [item[1] for item in scored_moves]

    # Διαχείριση του best_move_hint
    if best_move_hint:
    ...
    # Παράμετρος μείωσης βάθους (Reduction factor για NMP)
    R = 1 if depth < 5 else 2 

    # Έλεγχος αν η προηγούμενη κίνηση ήταν NULL
    is_prev_move_null = False
    if board.move_stack:
        is_prev_move_null = (board.move_stack[-1] == chess.Move.null())

    if maximizing:
        # --- NULL MOVE PRUNING (Maximizing) ---
