...
} // <--- ΑΥΤΗ Η ΑΓΚΥΛΗ ΛΕΙΠΕΙ Ή ΈΧΕΙ ΣΒΗΣΤΕΙ ΚΑΤΑ ΛΑΘΟΣ!
...
int alphabeta(int alpha, int beta, int depth, chess::Board& board, int ply, chess::Move& best_move) {
    ...
    for (size_t i = 0; i < moves.size(); i++) {
        if (board.isCapture(moves[i])) {
            ...
            if (ply < 64 && moves[i] == killer_moves[ply][0]) move_scores[i] = 9000;
            else if (ply < 64 && moves[i] == killer_moves[ply][1]) move_scores[i] = 8000;
        }
    }
    ...
    for (size_t i = 0; i < moves.size(); i++) {
        ...
        // ⚡ ΑΠΟΛΥΤΗ ΔΙΟΡΘΩΣΗ: Έλεγχος promotion με βάση τους κανόνες του σκακιού (Zero-API Dependency)
        // Αν το κομμάτι είναι πιόνι και πηγαίνει στην πίσω γραμμή (Rank 1 ή Rank 8)
        auto moving_piece = board.at<chess::PieceType>(mv.from());
        int target_rank = mv.to().rank(); // Επιστρέφει 0 έως 7
