...
# Ο επίσημος πίνακας ασφάλειας Stockfish / Glaurung που βρήκατε
...
def calculate_king_safety(board, king_color):
    ...
    # 1. Ορισμός της Βασικής Ζώνης του Βασιλιά (King Zone)
    # Παίρνουμε τα 8 γειτονικά τετράγωνα + το τετράγωνο του ίδιου του Βασιλιά
    king_zone = set(board.attacks(king_square))
    king_zone.add(king_square)
    
    # Προσθέτουμε τα 3 τετράγωνα μπροστά από τη ζώνη (Facing Enemy Position)
    king_rank = chess.square_rank(king_square)
    king_file = chess.square_file(king_square)
    
    # Αν ο βασιλιάς είναι λευκός, κοιτάζει προς τα πάνω (ranks +1, +2), αν μαύρος προς τα κάτω (-1, -2)
    direction = 1 if king_color == chess.WHITE else -1
    
    # Έλεγχος για τις 3 στήλες (αριστερά, κέντρο, δεξιά του βασιλιά)
    for f_offset in [-1, 0, 1]:
        target_file = king_file + f_offset
        target_rank = king_rank + direction
        
        # Αν τα τετράγωνα είναι εντός σκακιέρας, τα προσθέτουμε στη ζώνη
        if 0 <= target_file <= 7 and 0 <= target_rank <= 7:
            king_zone.add(chess.square(target_file, target_rank))
            # Προσθήκη και ενός δεύτερου επιπέδου μπροστά για βαθύτερη ανίχνευση
    ...
    # 2. Έλεγχος εχθρικών επιθέσεων στη ζώνη
    for piece_type in [chess.KNIGHT, chess.BISHOP, chess.ROOK, chess.QUEEN]:
        ...
        for piece_square in pieces:
            ...
            if king_zone.intersection(piece_attacks):
                ...
                # Προσθήκη μονάδων επίθεσης κατά Stockfish
    ...
    # 3. Αν επιτίθενται λιγότερα από 2 κομμάτια, δεν υπάρχει στρατηγικός κίνδυνος
    if attacking_pieces_count < 2:
        return 0
        
    # Περιορισμός του index έως 99 για αποφυγή IndexError
    index = min(attack_units, 99)
    return STOCKFISH_SAFETY_TABLE[index]
...
def evaluate_board(board):
    ...
    # Υπολογίζουμε την ποινή μόνο αν δεν είμαστε σε πλήρες φινάλε
