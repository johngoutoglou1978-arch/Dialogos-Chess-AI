...
def evaluate_board(board, depth=0):
    ...
    # =========================================================================
    # 🌟 Η ΜΕΓΑΛΗ/ΒΑΡΙΑ ΓΝΩΣΗ ΕΚΤΕΛΕΙΤΑΙ ΜΟΝΟ ΣΤΑ ΡΗΧΑ ΒΑΘΗ (DEPTH-GATE) 🌟
    # =========================================================================
    if depth >= 3:
        ...
        # --- ΕΝΣΩΜΑΤΩΣΗ ΝΕΑΣ ΓΝΩΣΗΣ: ΚΛΕΙΣΤΟ ΚΕΝΤΡΟ & ΚΑΚΟΙ ΑΞΙΩΜΑΤΙΚΟΙ ---
        if endgame_weight < 1.0:
            # Ανίχνευση κλειστού κέντρου με γρήγορο έλεγχο Bitmasks
            # Ελέγχουμε αν υπάρχουν τα πιόνια στα d4/d5 ή e4/e5 ακαριαία
            d4_mask = (1 << chess.D4)
            d5_mask = (1 << chess.D5)
            e4_mask = (1 << chess.E4)
            e5_mask = (1 << chess.E5)
            ...
            # Δημιουργία γρήγορων Bitmasks για τα κεντρικά τετράγωνα
            ...
            # Υπολογισμός Μπόνους για Σπασίματα στα Φτερά (Pawn Breaks)
            if center_is_closed:
                BREAK_BONUS = 30
...
def minimax(board, depth, alpha, beta, maximizing, nodes, start_time, time_limit, ply=0, best_move_hint=None, interrupt_queue=None):
    ...
    def get_ordered_moves():
        ...
        # Στάδιο 3: Όλες οι υπόλοιπες κινήσεις (Υπολογίζονται ΜΟΝΟ αν χρειαστεί)
        scored_moves = []
        for move in board.legal_moves:
            if move == hint or move == tt_move:
                continue
            
            killers = killer_moves[ply] if 'killer_moves' in globals() and ply < len(killer_moves) else None
            score_val = mvv_lva(move, board, tt_move=tt_move, killers=killers)
    ...
    # Κοινές μεταβλητές αναζήτησης
    static_eval = evaluate_board(board)
    in_check = board.is_check()
    R = 1 if depth < 5 else 2 
    is_prev_move_null = board.move_stack and (board.move_stack[-1] == chess.Move.null())
    ...
    if maximizing:
        ...
        for moves_searched, move in enumerate(get_ordered_moves()):
    ...
    # 6. --- ΕΓΓΡΑΦΗ ΣΤΟ TRANSPOSITION TABLE ---
