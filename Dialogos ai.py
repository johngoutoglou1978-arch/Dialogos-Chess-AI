...
def minimax(board, depth, alpha, beta, maximizing, nodes, start_time, time_limit, ply=0, best_move_hint=None, interrupt_queue=None):
    ...
    if depth == 0 or board.is_game_over():
        if depth == 0 or board.is_checkmate():
            ...
            # Αν το παιχνίδι τελείωσε με Stalemate / 50 κινήσεις και είμαστε κερδισμένοι, αποφεύγουμε την ισοπαλία
    ...
    # Αν η θέση έχει ήδη 2 επαναλήψεις, ΔΕΝ εμπιστευόμαστε το TT για να μην "κλειδώσει" η ισοπαλία
    ...
    if maximizing:
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if board.is_repetition(3):
                ...
                # Ποινή -300 αν είμαστε στο +3 (πάνω από 1.0) για να αποφύγει την επανάληψη
    ...
    # --- MINIMIZING PLAYER ---
        ...
        for moves_searched, move in enumerate(moves):
            ...
            if board.is_repetition(3):
                score = evaluate_board(board)
                # Αν ο Μαύρος κερδίζει (κάτω από -1.0), δίνουμε +300 για να μην δεχτεί ισοπαλία ο Λευκός
                score = 300 if score < -1.0 else 0
                line = []
            else:
                # --- LATE MOVE REDUCTIONS (LMR) ---
